 ...
 */
fun thirdDigit(number: Int): Int = (number / sqr(10)) % 10
...
fun accountInThreeYears(initial: Int, percent: Int): Double {
    val y1 = initial.toDouble() + (initial * percent / 100).toDouble()
    val y2 = y1 + y1 * percent / 100

    return (y2 + y2 * percent / 100)
}
...
fun numberRevert(number: Int): Int {
    val th = number % 10
    val se = number / 10 % 10
    val fi = number / 100 % 10

    return (th * 10 + se) * 10 + fi
}
